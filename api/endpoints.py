...
vk = Valkey(host=settings.VALKEY_HOST, port=6379, db=0, decode_responses=True)
...
async def job_status(job_id: str):
    ...
    if task and task.done():
        status = (
            "completed" if not task.cancelled() and not task.exception() else "failed"
        )
...
async def job_results():
    ...
    for fname in os.listdir(output_dir):
        if fname.endswith(".md"):
            image_urls = await vk.get(await vk.get(fname))
            processed_image_urls = (
                image_urls.split(",") if image_urls is not None else []
            )

            results.append(
                CrawlJobResult(
                    url=await vk.get(fname),
                    markdown_file=fname,
